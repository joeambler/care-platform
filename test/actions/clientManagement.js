'use strict'
const ClientController = require('../../src/api/controllers/clients')
...
function createClient (response, callback) {
  ...
  UserController.verifyJWT(req, null, 'Bearer ' + response.variables.JWT,
    () => ClientController.postUsersClients(req, req.res))
}
...
function getClient (response, callback) {
  ...
  req.res = HelperFunctions.resSkeleton(response, callback)
  UserController.verifyJWT(req, null, 'Bearer ' + response.variables.JWT,
    () => ClientController.getClient(req, req.res))
}
...
function deleteClient (response, callback) {
  ...
  req.res = HelperFunctions.resSkeleton(response, callback)
  UserController.verifyJWT(req, null, 'Bearer ' + response.variables.JWT,
    () => ClientController.deleteClient(req, req.res))
}
