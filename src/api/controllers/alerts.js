...
function postAlert (req, res) {
  ...
  Promise.all([alertType, emails]).
    then(([alertType, emails]) =>
      saveNewAlert(component, alertType, alertDetails).then(a =>
...
function saveNewAlert (component, alertType, alertDetails) {
  return models.sequelize.transaction(t =>
    models.Alert.create({details: alertDetails, date: new Date()},
      {transaction: t}).then(a => Promise.all([
...
function getEmails (component) {
  return new Promise((fulfill, reject) =>
    component.getClient({include: [{model: models.User, as: 'Users'}]}).
      then(c => {
          const emails = []
  ...
  return new Promise((fulfill, reject) => {
    component.getPermissions({
      ...
      if (p.length > 0) {
        ...
        reject(
          'The component does not have permission to use this alert type.')
      }
    }, reject)
  })
}
