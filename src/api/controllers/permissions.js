...
function requestPermissions (req, res) {
  ...
  Promise.all(checks).
    then(([validPrototypes, match, devicesComparison, existingPermissions]) => {
      ...
      addPermissionsAndDefinitions(newPermissions, devicesComparison,
        component).then(() => {
        success()
      }, serverError)
    }, serverError)
...
function permissionAndDevicesMatch (deviceDefinitions, permissions) {
  return new Promise((fulfill) => {
    ...
    if (devicePermissions.length === 0 &&
      typeof deviceDefinitions === 'undefined') return fulfill(true)
    ...
    const allCorrect = deviceDefinitions.length ===
      correctlyDefinedPermissions.length
...
function getDefinitionConflicts (deviceDefinitions) {
  return new Promise((fulfill, reject) => {
    ...
    const devicesInDb = []
    deviceDefinitions.forEach(d => devicesInDb.push(
      models.DeviceType.findOne({where: {type: d.type}})))

    Promise.all(devicesInDb).then((devices) => {
      for (let i = 0; i < devices.length; i++) {
        if (devices[i] == null) {
          deviceDefinitions[i].prototype = JSON.stringify(
            JSON.parse(deviceDefinitions[i].prototype))
          comparison.devicesToCreate.push(deviceDefinitions[i])
          continue
        }
        if (JSON.stringify(JSON.parse(devices[i].prototype)) !==
          JSON.stringify(JSON.parse(deviceDefinitions[i].prototype))) {
...
function addPermissionsAndDefinitions (
  permissions, devicesComparison, component) {
  return models.sequelize.transaction(function (t) {
    ...
    const promises = [
      addDeviceDefinitionsAndPermissions(t, devicePermissions,
        devicesComparison, component),
      addPermissionsByType(t, 'event', permissions, models.EventType,
        component),
      addPermissionsByType(t, 'alert', permissions, models.AlertType,
        component)]
...
function addDeviceDefinitionsAndPermissions (
  t, devicePermissions, devicesComparison, component) {
  return models.DeviceType.bulkCreate(devicesComparison.devicesToCreate,
    {transaction: t}).then((devices) => {
    devices = devices.concat(devicesComparison.compatibleDevices)
    return models.Permission.bulkCreate(devicePermissions, {transaction: t}).
      then(ps => {
        const promises = []
        ps.forEach(p => {
...
function addPermissionsByType (t, type, permissions, model, component) {
  ...
  permissionsOfType.forEach(p => {
    promises.push(model.findOrCreate({where: {type: p.name}, transaction: t}).
      spread(instance =>
        models.Permission.create({type: type}, {transaction: t}).then(p => {
          const promises = []

          promises.push(type === 'event'
            ? p.setEventType(instance, {transaction: t})
            : type === 'alert'
              ? p.setAlertType(instance, {transaction: t})
              : null)
          promises.push(component.addPermission(p, {transaction: t}))
          return Promise.all(promises)
        })))
  ...
  user.getClients({where: {id: clientId}, through: {where: {admin: true, tentative:false}}}).
    then(clients => {
      ...
      clients[0].getComponents({where: {id: componentID}}).then(components => {
        ...
        getComponentPermissionsFlat(components[0], tentative, false).
          then(permissions => {
  ...
  user.getClients({where: {id: clientId}, through: {where: {admin: true, tentative:false}}}).
    then(clients => {
      ...
      clients[0].getComponents({where: {id: componentID}}).then(components => {
        ...
        getComponentPermissionsFlat(components[0], !setTentative, true).
          then((flatPermissions) => {
            const nonExistentPermissions = []
            const permissionsToUpdate = []
            permissionsToChange.forEach(p => {
              const matching = flatPermissions.filter(
                fp => fp.type === p.type && fp.name === p.name)
